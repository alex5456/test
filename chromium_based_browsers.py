...
def save_results(browser_name, type_of_data, content):
    if not os.path.exists("data"):
        os.mkdir("data")
    if not os.path.exists("data/"+browser_name):
        os.mkdir("data/"+browser_name)
    if content is not None:
        open(f'data/{browser_name}/{type_of_data}.txt', 'w', encoding="utf-8").write(content)
        print(f"\t [*] Saved in data/{browser_name}/{type_of_data}.txt")
    else:
